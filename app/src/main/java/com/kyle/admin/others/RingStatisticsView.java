...
public class RingStatisticsView extends View {
    ...
    private String[] mTitles = new String[]{"item1","item2", "item3", "item4"};
...
public  float ratio = 8;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCount; i++) {
            ...
            if (angle <= 90) {
                centerX = startX - width / ratio;
                centerY = startY - height / ratio;
                endX = centerX - width /ratio;
                endY = centerY;
                textX = centerX - width / 8f;
            } else if (angle <= 180) {
                centerX = startX + width /ratio;
                centerY = startY - height /ratio;
                endX = centerX + width / ratio;
                endY = centerY;
                textX = centerX + width / 16f;
            } else if (angle <= 270) {
                centerX = startX + width / ratio;
                centerY = startY + height / ratio;
                endX = centerX + width / ratio;
            ...
            String percent = mTitles[i] + " - "+ df.format(f1) + "%";
    ...
    public void setPercentAndColors(String[] titles ,float[] percent, int[] colors) {
        if (percent != null && colors != null) {
            if (percent.length == colors.length  && percent.length == titles.length) {
                mTitles  = titles;
                mPercent = percent;
                mColors = colors;
            } else {
                throw new IllegalArgumentException("length of percent must equals length of colors equals length of titles");
            }
        }
    }
